...
option_list <- list(
  make_option(c("-f", "--files"), 
              type = "character", 
              default = NULL,
              help = "Path to file containing list of copy number files (required)", 
              metavar = "FILE"),
...
main <- function() {
  ...
  data <- map_dfr(files, function(f) {
    ...
    tryCatch({
      fread(f, header = TRUE, sep = "\t")  %>% 
        as.data.frame() %>%
        separate(name, sep = ":|-", into = c("chr", "start", "end"), 
  ...
  p <- ggplot(data) +
    geom_line(aes(x = pos, y = CN, group = sampleID),
              ...
              alpha = 0.5,
              linewidth = 0.1) +
    geom_segment(aes(x = start, y = CN, xend = end, yend = CN),
                 color = "black",
                 linewidth = 0.3) +
    labs(title = title,
         subtitle = paste("Region:", opt$region)) +
    ylab("Copy Number") +
